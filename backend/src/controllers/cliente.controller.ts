...
export const clienteController = {
  ...
  // --- Tarjetas ---
  async obtenerTarjetas(req: Request, res: Response) {
    try {
      const userId = (req as any).user.id;
  ...
  async agregarTarjeta(req: Request, res: Response) {
    try {
      const userId = (req as any).user.id;
      const { nombre, numero, expiracion, cvv } = req.body;
      const card = await prisma.card.create({
        data: { userId, nombre, numero, expiracion, cvv }
      });
  ...
  async eliminarTarjeta(req: Request, res: Response) {
    try {
      const { id } = req.params;
