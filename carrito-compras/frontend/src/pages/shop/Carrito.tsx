...
const Carrito = () => {
  ...
  const handleUpdateQuantity = (id: string, nuevaCantidad: number, nombre: string) => {
    const success = updateQuantity(id, nuevaCantidad);
    if (!success && nuevaCantidad > 0) {
      toast.error(`No hay suficiente stock de ${nombre}`);
    }
  };
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <div className="h-20 w-20 bg-gray-50 rounded-xl flex items-center justify-center text-3xl">
                  📦
                </div>
                <div>
              ...
              <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  <button 
                    onClick={() => handleUpdateQuantity(item.id, item.cantidad - 1, item.nombre)}
                    className="px-3 py-1 hover:bg-gray-50 text-gray-500 font-bold"
                  >
                    -
                  </button>
                  <span className="px-4 py-1 font-bold text-gray-700">{item.cantidad}</span>
                  <button 
                    onClick={() => handleUpdateQuantity(item.id, item.cantidad + 1, item.nombre)}
                    className="px-3 py-1 hover:bg-gray-50 text-gray-500 font-bold"
                  >
