...
const OrdenesAdmin = () => {
  ...
  const ordenesFiltradas = ordenes.filter(o => {
    const matchesEstado = filtroEstado === 'TODAS' || o.estado === filtroEstado;
    const searchLower = searchTerm.toLowerCase();
    const matchesSearch = 
      o.id.toLowerCase().includes(searchLower) ||
      (o.user?.nombre || '').toLowerCase().includes(searchLower) ||
  ...
  if (loading) return <LoadingSpinner message="Consultando base de órdenes..." />;

  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-4xl font-black text-gray-900 tracking-tight">Gestión de Órdenes</h1>
          <p className="text-sm font-bold text-gray-400 mt-2 uppercase tracking-widest">Monitorea y procesa pedidos</p>
        </div>
        <div className="flex flex-wrap gap-4 w-full md:w-auto">
          <div className="relative">
            <input
              ...
              className="w-full md:w-64 pl-10 pr-4 py-2 border rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
              value={searchTerm}
          ...
          <select 
            ...
            className="border-2 border-gray-100 rounded-xl px-4 py-2 bg-white shadow-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold text-gray-600"
          >
          ...
          <button 
            onClick={exportarCSV}
            className="bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition flex items-center justify-center gap-2 shadow-lg shadow-green-100 font-bold"
          >
            📊 CSV
          </button>
          ...
          {canCreate && (
            <button 
              ...
              className="bg-blue-600 text-white px-6 py-2 rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-100 font-bold"
            >
              + Nueva Orden
            </button>
          )}
