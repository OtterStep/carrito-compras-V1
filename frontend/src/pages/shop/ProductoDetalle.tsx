...
const ProductoDetalle = () => {
  ...
  const [cantidad, setCantidad] = useState(1);
    ...
    const availableStock = producto ? (existingInCart ? producto.stock - existingInCart.cantidad : producto.stock) : 0;
  ...
  const handleAddToCart = () => {
    ...
    // Llamamos a addItem múltiples veces o modificamos addItem para aceptar cantidad
    // Dado que addItem en cartStore.ts actualmente solo agrega de a 1, 
    // lo ideal sería actualizar cartStore para que acepte cantidad.
    // Pero para minimizar cambios, podemos hacer un loop o actualizar el store.
    // Vamos a actualizar el store para que addItem acepte cantidad opcional.
    
    for (let i = 0; i < cantidad; i++) {
      addItem(producto);
    }
    
    toast.success(`${producto.nombre} agregado al carrito (${cantidad})`);
    setCantidad(1);
  };
        ...
        <div className="flex flex-col">
          ...
          {producto.stock > 0 && (
            <div className="flex flex-col gap-2 mb-8">
              <label className="text-sm font-bold text-gray-500 uppercase tracking-wider">Cantidad</label>
              <div className="flex items-center gap-4">
                <div className="flex items-center border-2 border-gray-100 rounded-2xl overflow-hidden">
                  <button 
                    onClick={() => setCantidad(Math.max(1, cantidad - 1))}
                    className="px-6 py-3 hover:bg-gray-50 text-gray-500 font-bold transition-colors disabled:opacity-30"
                    disabled={cantidad <= 1}
                  >
                    -
                  </button>
                  <span className="px-8 py-3 font-black text-xl text-gray-800 min-w-[60px] text-center">{cantidad}</span>
                  <button 
                    onClick={() => setCantidad(Math.min(availableStock, cantidad + 1))}
                    className="px-6 py-3 hover:bg-gray-50 text-gray-500 font-bold transition-colors disabled:opacity-30"
                ...
                {existingInCart && (
                  <span className="text-xs text-orange-500 font-bold">
                    ({existingInCart.cantidad} ya en el carrito)
                  </span>
                )}
              </div>
              {availableStock <= 0 && (
                <p className="text-xs text-red-500 font-bold">Has alcanzado el límite de stock disponible</p>
              )}
            </div>
          )}
          ...
          <div className="flex gap-4">
            <button 
              ...
              disabled={producto.stock <= 0 || availableStock <= 0}
            ...
            >
              {availableStock <= 0 && producto.stock > 0 ? 'Sin stock disponible' : 'Agregar al Carrito'}
            </button>
            <button 
