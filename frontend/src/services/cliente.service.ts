...
export const clienteService = {
  ...
  // Tarjetas
  getTarjetas: async () => {
    const response = await api.get('/cliente/tarjetas');
    return response.data;
  },
  agregarTarjeta: async (data: any) => {
    const response = await api.post('/cliente/tarjetas', data);
    return response.data;
  },
  eliminarTarjeta: async (id: string) => {
    const response = await api.delete(`/cliente/tarjetas/${id}`);
    return response.data;
  },
